...
function mainAlex(NN=20,TT=8000,pb=0.01,cb=1,cr=4)
    Ratios = Array(Float64,20,8)
    filename = "ScatterPlots/x2ImportTest_";
    AA = dynamicCommunicators(NN,TT,pb,cb,cr,filename,Ratios)
...
function TimeSeries(Ratios)
    plot(Ratios)
    xlabel!("time")
    ylabel!("broadcast/degout")
    title!(string("node num = 20"))
    savefig("ScatterPlots/Cbroad-Deg-Time")
end
...
function dynamicCommunicators(NN,TT,pb,cb,cr,filename,Ratios)
