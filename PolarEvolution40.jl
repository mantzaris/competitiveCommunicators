...
function mainAlex(NN=40,TT=24000,pb=0.01,cb=1,cr=4)
...
function multiHistogram(data,file,title)
    ...
    groupedbar(data, labels=labels, nbins=40)
    xlabel!("Nodes")
...
function heatMap(AA,file,flag)
        xs = [string(x) for x in 1:40]
	ys = [string(x) for x in 1:40]
	heatmap(xs,ys,AA);
	title!(string("Send vs Recieve ", (flag==1) ? "Non Polar" : "Polarized", "Nodes = 40"))
	savefig(file)
	println("Heat Map")
end
...
function dynamicCommunicators(NN,TT,pb,cb,cr,MessagesFired,ResponsesTriggered)
    ...
    Messages = Array{Float64}(40)
    data = Array{Float64}(40,2)
    Avg = Array{Float64}(24,2)
    d = 1
    f = 1
    AA = zeros(NN,NN,TT)
    i=1;
    ...
    for ll in 1:24
    	Avg[ll,1] = (sum(MessagesFired[ll,31:40])/10)/(sum(MessagesFired[ll,11:20])/10)
        Avg[ll,2] = (sum(MessagesFired[ll,21:30])/10)/(sum(MessagesFired[ll,1:10])/10)
    end
    
    TimeSeries(Avg,NN,"Plots/TopAvg-BotAvg-4Groups-40","Number of Nodes = 40 - Power Top 10/Middle 10 & Middle 10 / Bottom 10 vs Time")
    savefig("Plots/TopAvg-BotAvg-4Groups-40")
    ...
    heatMap(ResponsesTriggered[:,:,1], "Plots/ResponsesTriggered_1-8000-40",1);
    heatMap(ResponsesTriggered[:,:,2], "Plots/ResponsesTriggered_8001-16000-40",2);
    multiHistogram(data[:,:,1], "Plots/ResponsesTriggered_GroupedBar_Lowest-40", "Responses Relative to Lowest - Polarized to Non-Polarized - Nodes = 40")
